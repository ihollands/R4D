...
flights %>% 
  filter(dep_delay <= 20 & dep_delay >= 10 & origin == 'LGA')
```
...
3. Write two lines on why it is important to ungroup a grouped dataset (once your are done with the grouping operations).
  If you do not ungroup your data, then your subsequent operations on the data set will occur on the aggregated groupings, not the individual data. Grouping is a mutating change.
...
```{r}
flights %>% 
  mutate(dayName = weekdays(time_hour)) -> flightsWeek
```
...
```{r}
flights %>%
  filter(arr_delay > 0 & arr_delay < 100) %>% 
  ggplot(data = ., mapping = aes(arr_delay)) + geom_histogram(bins = 100)
```
